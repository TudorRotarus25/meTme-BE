...
class DailyLocationUpdate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'locationUpdate:daily';
     ...
     * @var string
     */
    protected $description = 'The location update for meetings that are not gonna happen in less than a day';
    ...
    {                                                                                                                                                                 //24h
        $usersQuery = DB::select('select u.user_id, m.id from meetings m join users_in_meetings u where eta IS NULL or TIMESTAMPDIFF(MINUTE, NOW(), from_time) - eta > 1440');
