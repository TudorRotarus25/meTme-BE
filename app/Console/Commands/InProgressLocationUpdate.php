...
class InProgressLocationUpdate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'locationUpdate:inProgress';
    ...
    {                                                                                                                                                                  
        $usersQuery = DB::select('select u.user_id, m.id from meetings m join users_in_meetings u where eta != 0 and TIMESTAMPDIFF(MINUTE, NOW(), from_time) - eta BETWEEN -60 AND 5');
